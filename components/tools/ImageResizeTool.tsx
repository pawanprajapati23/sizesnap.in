  ...
  const processImage = async (file: File) => {
    ...
    try {
      ...
      if (config.width || config.height) {
        if (config.width) options.maxWidthOrHeight = Math.max(config.width, config.height || 0)
      }

      const compressed = await imageCompression(file, options)

      // If specific dimensions needed, use Canvas
      if (config.width && config.height) {
      ...
      <div className="p-5 space-y-4">
        ...
        {status === 'processing' && (
          <div className="text-center py-10">
            ...
            <p className="font-semibold text-gray-700">Processing your image...</p>
