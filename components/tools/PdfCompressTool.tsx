...
import { Upload, Download, RefreshCw, CheckCircle, AlertCircle, FileText, ShieldCheck, Cpu, Sliders, Settings, Share2 } from 'lucide-react'
...
export default function PdfCompressTool({ config }: Props) {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      const file = new File([blob], originalFile.name || 'document.pdf', { type: 'application/pdf' })
      if (navigator.canShare && navigator.canShare({ files: [file] })) {
        await navigator.share({
          files: [file],
          title: 'My Compressed PDF - SizeSnap',
          text: 'I compressed my PDF using SizeSnap.in!',
        })
      } else {
